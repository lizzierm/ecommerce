<x-app-layout>
    <x-container class="mt-12">
        <div class="grid grid-cols-3 gap-6">
            <div class="col-span-2">
                @livewire('shipping-addresses')
            </div>

            <div class="col-span-1">
                <div class="bg-white rounded-lg shadow overflow-hidden mb-4">
                    <div class="bg-purple-400 text-white p-4 flex justify-between items-center">
                        <p class="font-semibold">
                            Resumen de compras ({{ Cart::instance('shopping')->count() }}) 
                        </p>

                        <a href="{{ route('cart.index') }}">
                            <i class="fa-solid fa-cart-shopping"></i>
                        </a>
                    </div>

                    <div class="p-4 text-gray-600">
                        <ul>
                            @foreach (Cart::content() as $item)
                                <li class="flex items-center space-x-4">
                                    <figure class="shrink-0">
                                        <img class="h-12 aspect-square" src="{{ $item->options->image }}" alt="">
                                    </figure>

                                    <div class="flex-1">
                                        <p class="text-sm">{{ $item->name }}</p>
                                        <p>Bs. {{ $item->price }}</p>
                                    </div>

                                    <div class="shrink-0">
                                        <p>{{ $item->qty }}</p>
                                    </div>
                                </li>
                            @endforeach
                        </ul>

                        <hr class="my-4">

                        <div class="flex justify-between">
                            <p class="text-lg">Total</p>
                            <p>Bs. {{ Cart::subtotal() }}</p>
                        </div>
                    </div>
                </div>

                @php
                    $user = Auth::user();
                    $hasShippingAddress = $user && $user->addresses()->exists();
                @endphp

                @if ($hasShippingAddress)
                    <a href="{{ route('checkout.index') }}" class="btn btn-purple block w-full text-center">
                        Siguiente 
                    </a>
                  
                @else
                    <div class="btn btn-purple block w-full text-center cursor-not-allowed">
                        Siguiente
                    </div>
                    <p class="text-red-500 text-center mt-2">
                        Debe añadir una dirección de envío para continuar.
                    </p>
                @endif
            </div>
        </div>
    </x-container>
</x-app-layout>
